...
Both icons come out of `Tools/AppIcon/make_icons.py`. **Do not hand-edit the
PNGs**, and re-run the script rather than touching a single variant.

The mark is a *valid clue*: a two-cell cage reading `6×`, holding 2 and 3. It is
a true statement about the game rather than decoration, it shows the one
artefact unique to this genre (a target in the corner of an outlined region),
and it carries the board's own thesis, which the previous abstract mark omitted
entirely: the cage boundary is solid, the division inside it is dotted.

Square corners and mitred joins, matching `BoardView` and `DESIGN.md` §1. The
old mark used round caps and joins, which contradicted the surface it stood for.

Standard is light, Pro is a neutral near-black. See `DESIGN.md` §3 for why both
the ground and the typeface moved away from the sibling app.

Three things that were learned by rendering rather than by reasoning, and will
be re-learned by anyone who edits the script:

- A system font renders a cap at roughly 0.72 of its nominal size, so sizing by
  point size lands a third too small. `size_for()` takes a target cap height.
- The multiplication sign is a hairline whose weight axis does not move it, so
  beside a bold numeral it reads as a smudge in a different colour. The operator
  is drawn as two strokes.
- Weight 400 does not survive 40pt.

**Check it at 40pt before committing.** An icon that only works at 1024 is not
an icon.
